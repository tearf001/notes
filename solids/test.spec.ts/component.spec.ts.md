...
import { createRoot, createComponent, mergeProps, splitProps, createUniqueId, createSignal, createEffect, JSX} from "../src/index.js";
...
const Comp2 = (props: { greeting: string; name: string; optional?: string }) => {
  ...
  expect((p as any).name).toBeUndefined(); // pick ["greeting", "optional"] from props
  expect((q as any).greeting).toBeUndefined(); // now its missing
...
describe("CreateComponent", () => {
  ...
  test("null/undefined props are replaced with empty props", () => {
    createRoot(() => {
      ...
      nonObjects.forEach(nonObject => { // identity func. 返回的是 {}
...
describe("mergeProps", () => {
  ...
  it("skips undefined values", () => {
    ...
    const props = mergeProps(a, b, c, d); // proxy or 
    expect(props.value).toBe(1); // proxy eval
    bValue = 2;
