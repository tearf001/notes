...
mutable struct Column
    ...
    # lazily/manually initialized fields # 延迟或者手工初始化的列
...
function Column(type::Type, options::Union{Parsers.Options, Nothing}=nothing)
    ...
    col = Column(type === Missing ? HardMissing : T, # type
        type >: Missing, # anymissing
        type !== NeedsTypeDetection, # userprovidedtype
        false, NaN, false) # no willdrop, NaN pool, no columnspecificpool
    if options !== nothing
        col.options = options # 延迟初始化
    end
    return col
end
...
function Column(x::Column)
    @assert isdefined(x, :lock) # 申明列定义已经有锁定义
    y = Column(x.type, x.anymissing, x.userprovidedtype, x.willdrop, x.pool, x.columnspecificpool) # 复制
    y.lock = x.lock # parent and child columns _share_ the same lock 共享锁定义
    if isdefined(x, :options)
        y.options = x.options # 共享选项
    end
    # specifically _don't_ copy/re-use x.column; that needs to be allocated fresh per parsing task # 不要拷贝和重新使用原列定义, 那需要刷新分配每个解析任务
    return y
end

"""
    isvaliddelim(delim) Whether a character or string is valid for use as a delimiter.
"""
isvaliddelim(delim) = false
isvaliddelim(delim::Char) = delim != '\r' && delim != '\n' && delim !='\0' #不允许的符号
isvaliddelim(delim::AbstractString) = all(isvaliddelim, delim) # 全有效的字符组合
...
"""
function checkvaliddelim(delim) # delim定义了,但是无效, 抛出异常
    delim !== nothing && !isvaliddelim(delim) &&
        throw(ArgumentError("invalid delim argument if '\\r', '\\n', '\\0'"))
end
...
function checkinvalidcolumns(dict::AbstractDict, argname, ncols, names)
    for (k, _) in dict
        if k isa Integer
            ...
            isvalid = (k isa Regex && any(nm -> contains(string(nm), k), names)) || Symbol(k) in names # k要么是name, 要么是包含name的正则表达式
...
function initialize_columns(ncols::Int, types, names, args...; validate)
    columns = Vector{Column}(undef, ncols) # 初始化n个元素的Column向量
    customtypes = Tuple{}
